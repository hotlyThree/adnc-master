...
#pragma warning restore S125 // Sections of code should not be commented out
            ...
            {
                throw new DBConcurrencyException("当前数据已被删除或不存在"); // 自定义并发异常 
            }
        }
        ...
        public IQueryable<TResult> LeftJoin<TInner, TKey, TResult>(IEfRepository<TInner> inner, Expression<Func<TEntity, TKey>> outerKeySelector, Expression<Func<TInner, TKey>> innerKeySelector, Expression<Func<TEntity, IEnumerable<TInner>, TResult>> resultSelector) where TInner : EfEntity
        {
            return DbContext.Set<TEntity>().GroupJoin(
            inner.GetEntity(),
            outerKeySelector,
            innerKeySelector,
            (outerResult, innerResults) => new { outer = outerResult, InnerResults = innerResults })
            .SelectMany(
                x => (x.InnerResults.DefaultIfEmpty() != null)? x.InnerResults.DefaultIfEmpty().Select(innerResult => new { outer = x.outer, inner = innerResult })
                        : new[] { new { outer = x.outer, inner = (TInner)null } },
                (x, y) => resultSelector.Compile()(x.outer, y.inner != null ? new[] { y.inner } : null));
        }
        public IQueryable<TResult> LeftJoin<TInner, TKey, TResult>(
        IQueryable<TInner> inner,
        Expression<Func<TEntity, TKey>> outerKeySelector,
        Expression<Func<TInner, TKey>> innerKeySelector,
        Expression<Func<TEntity, IEnumerable<TInner>, TResult>> resultSelector) where TInner : EfEntity
        {
            return DbContext.Set<TEntity>().GroupJoin(
                inner,
                outerKeySelector,
                innerKeySelector,
                (outerElement, innerElements) => new { outerElement, innerElements })
                .Select(x => resultSelector.Compile()(x.outerElement, x.innerElements));
        }
