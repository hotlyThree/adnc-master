    ...
    protected override async Task<CacheValue<T>> BaseGetAsync<T>(string cacheKey, Func<Task<T>> dataRetriever, TimeSpan expiration)
    {
        ...
        try
        {
            ...
            if (item != null)
            {
                ...
                //var nullflag = type.IsGenericType;
                //数组对象
                if (item is IEnumerable co && co.Cast<object>().Any())
                {
                    await SetAsync(cacheKey, item, expiration);
                    return new CacheValue<T>(item, true);
                }
                //值或非空对象
                else if (type.IsValueType || (type.IsGenericType && item.GetType().GetGenericTypeDefinition() == typeof(Nullable<>)))
                {
                    await SetAsync(cacheKey, item, expiration);
                    return new CacheValue<T>(item, true);
                }
